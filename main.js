...
//   return inNewYork;
// });
// console.warn(newYorkBusinesses);

const printNY = () => {
  const zip = 'addressZipCode';
  newYorkBusinesses.forEach(element => {
    outEl.innerHTML += `<h2>${element.companyName}</h2>
         <section>
           ${element.addressFullStreet}
         </section>
         <section>
          ${element.addressCity}, ${element['addressStateCode']}, ${element[zip]}
         </section> 
       `;
        outEl.innerHTML += "<hr/>";
  });
...
const printManufacturing= () => {
  const zip = 'addressZipCode';
  manufacturingBusinesses.forEach(element => {
    outEl.innerHTML += `<h2>${element.companyName}</h2>
         <section>
           ${element.addressFullStreet}
         </section>
...
document
    ...
    .addEventListener("keypress", keyPressEvent => {
        if (keyPressEvent.charCode === 13) {
            ...
            const foundBusiness = businesses.find(
                business => {
                  const agentName = [business.purchasingAgent.nameFirst, business.purchasingAgent.nameLast];
                   return agentName.includes(keyPressEvent.target.value); 
                });
    ...
    let totalRainFall = monthlyRainfall.reduce((currentVal, nextVal) => currentVal += nextVal);
